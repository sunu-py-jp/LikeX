...
各モジュールはパッケージとして導入でき、ソースフォルダをコピーして使うこともできます。Explorer・Spreadsheet・LikeSlideは小さな共通基盤 `@likex/core` を使います。共通Providerは不要です。
...
React / React DOMと表示枠の高さは利用側で用意します。生成済みCSSを同梱しているため、利用先へのTailwind CSSの導入は不要です。コピー導入ではUIの `src/` と `packages/core/src/` を隣接フォルダへ置き、UI側の `core.ts` を相対importに変更します。SpreadsheetとLikeSlideは `ooxml.ts` も変更します。詳細は各導入ガイドを参照してください。
...
デモは `http://127.0.0.1:5173/`（Explorer）、`http://127.0.0.1:5173/spreadsheet`（Spreadsheet）、`http://127.0.0.1:5173/slide`（LikeSlide）で起動します。保存先はタブ内メモリです。SpreadsheetとLikeSlideはExcel・PowerPointの全機能との互換を保証するものではありません。
