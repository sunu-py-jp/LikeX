...
async function testConsumer(module) {
  const { artifactRoot, packageRoot, sourceRoot, npmCacheRoot } = libraryModule(module);
  ...
  try {
    ...
    const styles = await checkConsumerStyles(consumer, { module, cssFile: path.join(copiedSource, 'styles.css'),
    ...
    const report = {
      source: `packages/${module}/src copied unchanged to components/${module} in a temporary project outside the repository`,
      ...
      ssrBytes: ssr.renderedBytes, stylesheetImport: `components/${module}/styles.css`, ...styles, ...nextStyles,
