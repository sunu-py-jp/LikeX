...
export function inspectDocumentImage(input: unknown): { src: string; bytes: number; width: number; height: number } {
  const fail = (): never => { throw new Error("Images must contain valid embedded PNG or JPEG data within the size limits."); };
  if (typeof input !== "string" || input.length > 40 + Math.ceil(DOCUMENT_LIMITS.imageBytes / 3) * 4) return fail();
  ...
  if (binary.length > DOCUMENT_LIMITS.imageBytes || btoa(binary) !== payload) return fail();
  ...
  if (width < 1 || height < 1 || width > DOCUMENT_LIMITS.imageDimension || height > DOCUMENT_LIMITS.imageDimension || width * height > DOCUMENT_LIMITS.imagePixels) return fail();
