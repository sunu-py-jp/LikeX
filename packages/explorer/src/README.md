...
| 本文・セマンティック検索を接続する | [検索の実行タイミングと公開型](#search-integration)、同節の本文キャッシュ・検索API例 |
...
仮想化するのは一覧のDOMです。全メタデータ、追加した `File` への参照、保存差分はクライアントで保持します。フォルダツリー、保存ペイロード、外部の重い `renderIcon` / `onEvent`、ファイルのダウンロードやZIP作成まで定数時間・定数メモリにするものではありません。サーバー側検索は [`onSearchRequest`](#search-integration) へ接続できますが、結果として表示できるのは読み込んだ一覧の項目です。大量データのページ取得や未読込項目の追加は、親のデータ取得・一覧更新で管理します。
