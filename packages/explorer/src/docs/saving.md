...
| OS・ブラウザの閉じるボタン、再読み込み、別文書への移動（ブラウザの戻る・進むを含む） | ブラウザ標準の確認を要求します。子を閉じても共有の下書きは親に残ります。親文書の終了ではメモリ上の下書きも失われます。 |
...
マウスのサイドボタンも、[Explorer内のフォルダ履歴](./configuration.md#マウスのサイドボタンで戻る進む)を移動する間は確認しません。その方向の履歴がなくブラウザの戻る・進むに任せた場合は、別文書への移動なら `warnOnUnsavedChanges`、同じ文書内のSPA遷移なら親のルーティング側で確認を扱います。
