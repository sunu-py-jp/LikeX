...
読み取り専用、機能のON/OFF、選択方式、表示形式、サイドバー、マウスとキーボード操作を設定します。
...
| `mouseNavigation` | マウスのサイドボタンで現在のタブのフォルダ履歴を戻る・進む操作。既定 `true`。その方向に履歴がなければブラウザー標準の動作を維持します。 |
...
サイドボタンを常にブラウザー側の操作に使う場合は、次のように無効にします。Explorerのツールバーの戻る・進むボタンと、Alt + ← / → のキーボード操作は引き続き使えます。

```tsx
<Explorer initialEntries={savedEntries} features={{ mouseNavigation: false }} />
```

Explorerのフォルダ履歴とブラウザーのHistory APIは同期しません。履歴の端からブラウザーがページを離れる場合の未保存確認は、[未保存の変更がある状態で画面を離れる](./saving.md#unsaved-changes)を参照してください。

ダイアログ・プレビュー・詳細の表示中、名前の編集中、保存などの処理中は、操作対象を保つためサイドボタンによるExplorer内の移動を行いません。この場合もブラウザー標準のイベントは抑止しません。

この操作には、ブラウザーがサイドボタンを `button: 3`（戻る）/ `button: 4`（進む）のマウスイベントとしてExplorerへ届ける必要があります。マウスのドライバーやブラウザーの設定が独自に処理する場合などは、Explorer内の移動に使えないことがあります。
