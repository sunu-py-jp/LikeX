...
フォルダの貼り付け・ドロップでは、右下に回転するインジケーターと検出済みのファイル数を表示します。階層を調べている間は「35ファイルを検出」、総数が分かってからの大量ファイルの確認・追加準備では「35 / 100ファイル」のように更新します。追加準備の分母は、拡張子・サイズの条件を満たしたファイル数です。フォルダ選択も、ブラウザーからファイル一覧を受け取った後の確認・追加準備で同じ進捗を表示します。OSの選択画面内でブラウザーが調べている段階の件数は取得できません。

進捗メッセージの「中止」で、そのウィンドウの取り込みを取り消せます。検出・確認・追加準備や編集許可の待機を終了し、今回の仮表示だけを削除します。既存のファイルや、それまでの未保存の変更は保持します。別ウィンドウの取り込みは継続します。「×」はメッセージを閉じるだけで、取り込みは中止しません。ブラウザーですでに始まったファイル読み取りを止められない場合も、中止後に届いた結果は追加しません。保存先へのアップロードを親が `onSave` で行っている間の通信中止は、このボタンの対象外です。

読み込めた項目から、取り込みを始めたウィンドウに元のフォルダ階層を保って仮表示します。追加先には直下のフォルダ・ファイルだけを並べ、取り込み中のフォルダも開いて行き来できます。同名の既存フォルダにはまとめて表示し、別のフォルダ行を追加しません。処理中のファイルと、そのファイルを含む親フォルダのアイコン上には回転するインジケーターを重ねます。一覧には「取り込み中」という文字を付けず、件数や進捗はメッセージ領域で確認できます。

仮表示のファイルは選択・編集・プレビュー・右クリック操作・保存の対象には含めず、検索結果にも表示しません。既存ファイルを上書きする場合も、確認が終わるまで元のファイルを保持します。取り込みが完了すると、開いているフォルダをそのまま確定後のフォルダへ引き継ぎます。途中で中止・失敗した場合、仮のフォルダを開いていれば、残っている最も近い親フォルダへ戻ります。

仮フォルダ内への新規作成・追加アップロードは、取り込みが完了してから利用できます。仮フォルダを開いたタブも、取り込みが終わってから別ウィンドウへ分離できます。

進捗と仮表示はローカルのファイル情報と階層の取り込み状況です。内容の転送や保存は行わず、すべての検証・上書き確認・編集許可が終わってから、ドラフトへ一括反映して他のウィンドウにも共有します。途中で中止・失敗した場合は、その回の仮表示を取り消し、既存データを保持します。保存・破棄・再読み込みなどで取り込みが中止された場合も同様です。ブラウザーがフォルダの情報を提供しない場合は、フォルダ選択から追加してください。

取り込みを確定した未保存のファイルには、アイコンの右上に青い点を表示します。対象は、ローカルの `File` を持つもののうち、保存の比較元から新規追加された、または本体が変わったファイルと、現在の階層の祖先フォルダ・ルートです。移動・名前変更・お気に入りだけの変更では表示しません。保存の完了待ちや失敗時は残り、保存に成功すると `onSave` が一覧を返さない場合も消えます。設定用のpropは不要です。[青い点と処理中インジケーターの違い](./appearance.md#unsaved-local-files)を参照してください。

アップロード後など、親側で実行する処理にも回転するインジケーターを使いたい場合は、[`processingEntryIds`](./appearance.md#processing-entries) に処理中のファイルIDを渡します。祖先フォルダへの表示もExplorerが計算するため、親側でフォルダのフラグを管理する必要はありません。
...
大量ファイルを扱う独自UIでは `await prepareAddAsync(files, parent, decisions, session, { signal, onProgress })` も使えます。同じ検証と確認結果を使い、画面の描画へ定期的に処理を譲りながら準備します。戻り値と競合エラーは `prepareAdd()` と共通で、`commit()` までは一覧を変更しません。`signal` で中止でき、準備完了後でも中止された結果の `commit()` は何も変更しません。反映済みの変更は中止によって取り消しません。

`onProgress` には公開型 `ExplorerImportProgress` の `{ phase, completed, total? }` を渡します。`prepareAddAsync()` の `phase` は情報確認の `"checking"` と追加準備の `"preparing"`、`completed` と `total` はその段階のファイル件数です。標準のフォルダ取り込みで使う同じ型には、フォルダ列挙の `"discovering"` もあり、総数が分かるまで `total` を省略します。更新は間引くため、すべての件数をコールバックするわけではありません。
