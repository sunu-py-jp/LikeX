...
アップロード後など、親側で実行する処理にも同じインジケーターを使いたい場合は、[`processingEntryIds`](./appearance.md#processing-entries) に処理中のファイルIDを渡します。祖先フォルダへの表示もExplorerが計算するため、親側でフォルダのフラグを管理する必要はありません。
