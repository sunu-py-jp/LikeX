...
フォルダの貼り付け・ドロップでは、右下に回転するインジケーターと検出済みのファイル数を表示します。階層を調べている間は「35ファイルを検出（総数を確認中）」、総数が分かってからの大量ファイルの確認・追加準備では「35 / 100ファイル」のように更新します。追加準備の分母は、拡張子・サイズの条件を満たしたファイル数です。フォルダ選択も、ブラウザーからファイル一覧を受け取った後の確認・追加準備で同じ進捗を表示します。OSの選択画面内でブラウザーが調べている段階の件数は取得できません。

読み込めたファイルから、取り込みを始めたウィンドウの追加先フォルダの一覧に「取り込み中」として仮表示します。フォルダ内のファイルは相対パスで場所を確認できます。仮表示は選択・編集・プレビュー・右クリック操作・保存の対象には含めず、追加先と異なるフォルダや検索結果には表示しません。既存ファイルを上書きする場合も、確認が終わるまで元のファイルを保持します。
