...
| 操作 | 方法 |
| --- | --- |
| 追加 | シートタブの右の「＋」を押す。300行×26列の空シートを追加。 |
| 名前変更 | タブを右クリックして「名前の変更」を選ぶか、選択済みタブをもう一度クリック。Enterまたはフォーカス移動で確定、Escapeで取り消す。 |
...
ドラッグ中はシート名の小さな半透明プレビューと挿入位置の縦線を表示します。タブ領域の左右端やその外側へポインターを動かして待つと、その方向へ横スクロールが続きます。ドロップで1回の並べ替えとして確定し、Escape、ドラッグの中止、フォーカス喪失、編集可否やブックの変更でプレビューとスクロールを取り消します。
...
| コマンド | 引数・結果 |
| --- | --- |
| `sheets.add` | `name?`。省略時は未使用の `Sheet1`、`Sheet2` など。300行×26列の空シートを末尾へ追加し、結果に新しい `sheetId` を返す。 |
