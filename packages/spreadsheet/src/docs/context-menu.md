# セル・シート・画像・図形の右クリックメニュー

[利用ガイドへ戻る](./README.md)

セル、行番号、列名、シートタブ、画像・図形・テキストボックスには既定の右クリックメニューがあります。`getContextMenuItems(context)` で独自の項目を追加でき、条件に合わなければ `[]` を返します。独自の項目は、行・列・シートの削除や「すべてクリア」の上に表示します。既定の項目は `getContextMenuItems` を指定しなくても利用できます。
...
| セル | 挿入…、削除…、コメントを挿入／編集／削除、すべてクリア |
| 行番号 | 上に行を挿入、行の高さを指定／自動調整、行を削除 |
| 列名 | 左に列を挿入、列の幅を指定／自動調整、列を削除 |
| シートタブ | 名前の変更、複製、削除 |
| 画像・図形・テキストボックス | コピー、貼り付け、複製、左右反転、上下反転、回転をリセット、削除 |
...
セルの「挿入…」「削除…」では、セルをずらす方向か、行・列全体の操作を選びます。離れた複数範囲では利用できません。[方向と具体例](./cell-shifts.md)
...
画像・図形・テキストボックスの右クリックでは対象を選択し、元のセル選択範囲を保持します。「複製」は `copySpreadsheetDrawing` と `drawings.paste` を使い、右下へ16pxずらした新しいIDの描画を追加します。貼り付けは既存クリップボード経路、反転・回転リセットは描画型に対応する `images.update` / `shapes.update` / `textBoxes.update`、削除は `drawings.delete` を使います。変更は編集許可とUndo/Redoの対象です。反転・回転リセットは `features.resize`、コピー・貼り付け・複製は対応するクリップボード機能に従います。図形の切り取り・前後移動の項目は提供しません。
...
| 項目 | 内容 |
| --- | --- |
| `target` | 右クリックしたセル・行番号・列名・シートタブ・描画。次の型一覧を参照し、`target.kind` で判別します。 |
...
| `drawing` | `{ kind: "drawing", sheetId, drawingId, drawingType }`（`image`・`shape`・`text`） |

行・列・シート位置は0始まり、セルの `address` は `B3` などです。`address` は `cell` の場合だけ使えます。

メニューを開くだけではセルの選択範囲を変更しません。たとえばA1:A3を選択したままB3を右クリックすると、独自の項目に渡す `selection` はA1:A3、`target.address` はB3です。複数範囲のSUMを組み立てる場合、反映先が参照範囲に含まれないよう利用側で除外し、循環参照を避けてください。
...
非アクティブなシートタブを右クリックしても、表示中のシートと選択範囲は維持します。「名前の変更」「複製」「削除」とカスタム項目の `target.sheetId` は右クリックしたシートを指します。シートの並び順はタブのドラッグ＆ドロップで変更できます。
...
反映直前と非同期の編集許可取得後に、処理と対象が有効か再確認します。セル・行番号・列名が対象の場合は、行・列の追加削除、セル結合、シート構成の変更、再読み込み等が挟まると、座標が同じでも別の対象になり得るため反映を中止します。現状は安全側に判定し、別シートの構成変更やUndo/Redoでも再実行を求めます。描画の既定操作は、対象選択・シート・ブックが変わった場合に反映を中止します。描画の独自項目は構造リビジョンと描画ID・種類を確認します。シートタブが対象の場合はIDで存在を確認するため、並べ替えや名前変更後も同じシートへ反映でき、削除済みなら中止します。`reject-if-changed` はすべての対象でブックの変更を検出します。
...
メニューは矢印キー・Home/Endで項目を移動し、Enter/Spaceで選択、Escape・Tab・外側クリックで閉じます。セル・行番号・列名・シートタブ・描画上のShift+F10またはContext Menuキーでも開けます。反映の確認は表示領域の中央に表示し、Escapeや背景クリックでキャンセルできます。

公開型は `SpreadsheetContextMenuContext`、`SpreadsheetContextMenuItem`、`SpreadsheetContextMenuProvider`、`SpreadsheetContextMenuChange` です。Coreがメニュー定義と非同期実行の方針、Spreadsheetがセル・行列・シート・描画の対象、コマンド、表示を担当します。
