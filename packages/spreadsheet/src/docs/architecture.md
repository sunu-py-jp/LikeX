...
| `import/` | 上限付きZIP/XML読み込み、Excelのセル・書式・描画等をJSONへ変換。DOM・React・外部通信への依存なし |
| `state/use-spreadsheet-import.ts` | 取り込みの中止・内容確認・古い結果の拒否と、編集許可・下書き・Undoへの接続 |
| `ui/spreadsheet-file-controls.tsx` | 「ファイル」タブ、ファイル選択、置き換え・互換性の確認。変換処理は共通APIへ委譲 |
...
GUIのセル操作では、開始時の選択をコンポーネント内の履歴に記録します。空白を含む範囲、複数範囲の順序・方向・`kind`、アクティブセルを保持し、貼り付けでは実際の転送先範囲を記録します。Undo／Redoはこの記録を復元先のシート寸法へ補正して使い、元の記録は保持します。選択の記録を保存JSONや公開の画面なしセッションへ追加することはありません。外部APIコマンドの履歴はGUIの選択を記録せず、変更対象から選択を決めます。[操作別の復元ルール](./history-session.md#guiとの境界)を参照してください。
