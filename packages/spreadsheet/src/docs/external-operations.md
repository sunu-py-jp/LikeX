...
`shape` は `SpreadsheetShapeKind` 型です。長方形・角丸長方形・三角形・ひし形・線・ブロック矢印など、[図形の種類](./insertions-and-json.md#図形の種類)に一覧があります。更新ではIDやオブジェクトの種類は変更できません。別の種類のIDを渡した場合も失敗します。画像の `patch.resourceId` は、同じブック内にある既存画像リソースを参照します。
...
画像・図形・テキストボックスの挿入には `rotation: 30` のように時計回りの角度を指定できます。更新は `patch: { rotation: 45 }`、解除は `patch: { rotation: 0 }` です。負の角度や360°以上は0〜360°未満に正規化し、角度だけの変更もUndo／Redoの対象になります。`features.resize: false` のとき、既存オブジェクトの回転変更は拒否します。
...
- `execute` / `batch` とその非同期版はシートの切り替えやフォーカス移動を行いません。構造の変更で現在の選択が範囲外になる場合は補正します。
- `undo` / `redo` は、GUI操作の履歴なら操作時に記録した選択を復元します。外部APIコマンドの履歴ではGUI選択と操作対象が異なる場合があるため、セルの変更対象へ移動し、構造変更では現在の選択を有効な範囲へ補正します。どちらもコンポーネント外のDOMフォーカスは奪いません。[履歴操作後の選択](./history-session.md#guiとの境界)を参照してください。
