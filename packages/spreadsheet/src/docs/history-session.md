...
GUIの履歴上限は50操作です。保存成功後もUndo／Redoの両方の履歴をコンポーネントを閉じるまで保持します。Undo後に保存してもRedoは残り、その後に新しい変更を反映するとRedoを消します。再読み込み・変更の破棄・`onEditRequest` による別ブックへの置き換え・再マウントでは履歴をリセットします。保存後の未保存状態の判定と編集許可は[保存・編集許可・イベント](./lifecycle.md)を参照してください。
...
選択の記録はコンポーネント内の履歴にだけ保持し、保存JSONや公開の画面なしセッションには含めません。コンポーネント外の入力欄などにDOMフォーカスがある場合、そのフォーカスは奪いません。ブック内の選択更新と、キーボード入力先のフォーカス移動は区別します。保存成功・再読み込み・変更の破棄では画面状態をリセットしますが、保存成功時は履歴内の選択記録を保持します。
