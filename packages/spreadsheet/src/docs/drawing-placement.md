...
画像・図形・テキストボックスの移動とサイズ変更では、配置先の位置と大きさをそのままプレビューします。セル領域の端や外側へドラッグすると、その方向へ連続してスクロールし、ポインターを動かさずに表示範囲外まで操作できます。表示倍率を考慮して配置を計算します。

ポインターを離すと1回の変更として確定し、Undoできます。Escape、ポインターのキャンセル、ウィンドウのフォーカス喪失では未確定の変更を破棄します。操作中にシート・ブック・選択対象・編集許可・関連する機能設定・表示倍率が変わった場合も、以前の対象へ変更を適用しません。回転操作では自動スクロールしません。
