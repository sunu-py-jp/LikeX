...
ヘッダーの「形式を選択して貼り付け」から選びます。セル・行番号・列名の右クリックメニューには、通常の貼り付けに加えて「値のみ貼り付け」「書式のみ貼り付け」があります。通常の `Ctrl+V` / `⌘V` は「すべて」の貼り付けです。
...
| 書式のみ | このSpreadsheet内で最後にコピーしたセルの書式。値・数式・入力規則は維持。 |

数式のみでもコピー元の定数は貼り付けます。`$A$1` のような絶対参照は動かしません。値のみでは、計算結果が文字列の `00123` や `=文字列` であっても、その内容を文字列として保持します。数式機能OFFでも計算結果の値を貼り付けられます。

通常の貼り付けで書式・コメント・結合・入力規則を含めて転送できるのは、このSpreadsheet内でコピーし、ブラウザが内部コピー識別情報を維持している場合です。外部アプリや別のSpreadsheetからの貼り付けは、基本的にクリップボードのテキストを使用します。

「書式のみ」は、このSpreadsheetインスタンス内に保持したコピー内容を使い、OSクリップボードの読み取り権限を要求しません。外部アプリで新しくコピーしても、その外部の書式は取得せず、このSpreadsheet内で最後にコピーしたセルの書式を使います。コピー元がない場合は、先にこのSpreadsheet内のセルを `Ctrl+C` / `⌘C` でコピーする案内を表示します。

ヘッダー・右クリックからの通常／値のみ／数式のみの貼り付けはOSクリップボードを読み取ります。ブラウザに読み取りを拒否された場合は、古い内部コピーを代用せず、操作を中止して日本語の案内を表示します。`Ctrl+V` / `⌘V` では通常の貼り付けを実行できますが、値のみ・数式のみとは異なる操作です。

切り取りした範囲には通常の貼り付けを使います。部分的な形式だけを切り取り移動する操作は行わず、形式選択時には案内を表示してコピー元を維持します。コピー・切り取り・貼り付けは一つの連続範囲を対象とし、一度の貼り付けは10,000セルまでです。

行番号・列名から行・列全体を選んでコピーすると、横断する結合セルに合わせて範囲を広げません。コピー元で一部だけ含む結合の中は空セルとして扱います。貼り付け先では範囲をはみ出す結合とその内容を保護し、残りのセルへ貼り付けます。完全に含まれる結合は通常どおり転送します。切り取りは別で、結合の一部しか含まない場合は中止します。

行全体のコピーは貼り付け先の行の先頭列、列全体のコピーは貼り付け先の列の先頭行に合わせます。例えば、C列全体をコピーしてD100に貼り付けても、D1から配置します。

貼り付け後は、貼り付けた範囲全体を選択します。貼り付け前のアクティブセルがその範囲内なら同じ位置を保ち、範囲外なら左上へ移します。行・列全体の貼り付けでも、選択範囲の末端へ自動スクロールしません。

貼り付け先の行・列が足りない場合は、結合を含むコピーや切り取りでも、必要な行・列を末尾へ追加して貼り付けます。必要な `insertRows` / `insertColumns` がOFFの場合や、最大行数・列数を超える場合は全体を中止します。結合の一部だけに重なる場合の保護・拒否ルールは引き続き適用します。[末尾の自動拡張](./external-operations.md#貼り付け先の行列が足りない場合)にAPIの例があります。
...
`payload.values` は文字列の二次元配列です。値のみで型も維持したい場合は、計算結果の `displayedValues` と `valueTypes`（`string` / `number` / `boolean`）も渡します。`formats`、`validations` に同じ行列の書式・入力規則を指定できます。`source` は数式の相対参照を調整する基準です。コメントの二次元配列 `comments` と、コピー範囲の左上を基準にした結合範囲 `merges` も渡せます。既存セルからこれらをまとめて取得するには、[copySpreadsheetCells](./history-session.md#コピー貼り付け移動)を使います。

`cells.paste` には `partialMerges?: "reject" | "skip"` も指定できます。既定は `"reject"` です。`"skip"` では、貼り付け範囲を横断する既存の結合を内容ごと維持し、その部分への書き込みを飛ばします。コピー元の部分結合も除外する場合は、`copySpreadsheetCells` のオプションに同じ指定を渡します。既存値との競合を指定する `onConflict` とは別の設定です。[結合セルのコピー・貼り付け](./merged-cells.md#コピー貼り付け)に対応範囲をまとめています。

## 画像・図形・テキストボックスのコピー

オブジェクトを選択して `Ctrl+C` / `⌘C`、続けて `Ctrl+V` / `⌘V` を押すと、選択オブジェクトの右・下へ16pxずらして複製します。セルを選択してから貼り付けると、アクティブセルの左上へ配置します。幅・高さ・書式・文字を維持し、画像では画像データもコピーします。読み取り専用ではコピーだけを使えます。

この操作は同じSpreadsheet内のコピー情報を使います。別のSpreadsheetインスタンス間のGUI転送、Excelなど外部アプリの図形、OSクリップボードの画像ファイルの取り込み、図形の切り取り・形式選択貼り付けは対象外です。画像ファイルの挿入には「挿入 → 画像」を使います。図形・テキストボックスの文字を編集中は、通常のテキストのコピー・貼り付けになります。

新しい描画IDを付けて複製する処理は、GUIと公開コマンド `drawings.paste` で共通です。別ブックへの複製やブラウザなしの処理には、[公開のコピー用データとコマンド](./external-operations.md#図形画像テキストボックスの複製)を使えます。

GUIのコピー・貼り付けは `onEvent` の `type: "clipboard"` で通知します。オブジェクトの操作では任意項目 `drawingId` に対象のIDが入り、貼り付けでは新しいIDを通知します。
...
セル表示領域の端や外側へドラッグして待つと、その方向へスクロールが続き、見える範囲に合わせて塗りつぶす範囲の枠を更新します。拡大・縮小中もセル位置を正しく対応させます。離した位置と修飾キーで確定し、1回のUndoで戻せます。離す前のEscape、ポインターのキャンセル、ウィンドウのフォーカス喪失では反映しません。ドラッグ中に別の編集を開始したり、選択・表示倍率・編集可否を変えたりした場合も中止します。ドラッグ中や非同期の編集許可待ちにデータが変わった場合も、古い範囲へ書き込まないよう中止します。
