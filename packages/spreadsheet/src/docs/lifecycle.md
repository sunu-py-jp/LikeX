...
保存成功後もUndo／Redoの両方の履歴を保持します。`onSave` が正規化後のブックを返した場合も同様で、採用したブックが未保存状態の判定基準になります。たとえば値をAからBに変更して保存し、UndoでAへ戻すと `dirty: true`、Redoで保存済みのBへ戻すと `dirty: false` です。Undo後の保存ではRedoを消さず、新しい変更を反映したときに消します。
...
保存成功で編集セッションは終了します。保存後の最初のUndo／Redoでも、通常の編集と同様に必要な編集許可を取り直します。`onEditRequest` が拒否した場合は、ブックと履歴を変更しません。

`{ allowed: true, workbook: latestWorkbook }` を返すと、未保存の確定変更がない場合だけ最新ブックを基準にできます。異なるブックへ置き換える場合はUndo／Redoの履歴をリセットします。行列には安定したIDがないため、GUIは置き換え後に古い座標の操作を自動反映せず、再操作を求めます。外部APIの明示的な座標は、許可後のブックに対して解釈します。
...
更新成功・変更の破棄ではUndo／Redoの履歴もリセットします。履歴はメモリ内だけに保持し、コンポーネントのアンマウントやページ再読み込みで失われます。
