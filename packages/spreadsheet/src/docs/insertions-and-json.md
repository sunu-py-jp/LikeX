...
`images.insert` / `shapes.insert` / `textBoxes.insert` には `flipX` / `flipY` を指定できます。既存オブジェクトは対応する `update` コマンドの `patch: { flipX: true }` などで変更し、`false` で解除します。`features.resize: false` は既存オブジェクトの反転変更も拒否します。同じ状態の再指定、新規挿入時の指定、反転済みオブジェクトの貼り付けは、初期サイズと同様に許可します。
