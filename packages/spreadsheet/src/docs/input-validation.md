...
| `date` | `min?` / `max?` は `YYYY-MM-DD`。セルは実在するISO日付、またはExcelの1900日付システムのシリアル値を受け付けます。`DATE` などの数式が返す数値も同じ基準で検証します。 |
| `checkbox` | `"TRUE"` / `"FALSE"`。GUI操作でこれらの文字列を設定します。 |

日付は最小値・最大値を含めて比較します。シリアル値の小数部分は時刻として比較するため、最大日付に0.5日を加えた値は範囲外です。数値を受け付けても保存値や表示形式を自動変更しません。文字列書式や先頭アポストロフィで明示した数字列は、日付の数値として扱いません。

共通設定の `allowBlank` は既定 `true`、`message` は入力エラー時の説明です。最小値・最大値はそれぞれ省略可能です。カスタム数式による入力規則には対応していません。

画面からの入力エラーは、下部の選択セル情報と同じステータスバーに表示します。別の行を増やさず、閉じると通常のセル情報に戻ります。長い文は1行で省略し、マウスを重ねると全文を確認できます。
