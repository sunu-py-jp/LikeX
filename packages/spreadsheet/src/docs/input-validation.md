...
| `date` | `min?` / `max?` は `YYYY-MM-DD`。セルにも同じ形式の実在する日付を入力します。`DATE` などの数式が返す数値は1900日付システムのシリアル値として検証します。 |
| `checkbox` | `"TRUE"` / `"FALSE"`。GUI操作でこれらの文字列を設定します。 |

日付の数式結果は最小値・最大値を含めて比較します。時刻に相当する小数部分も比較するため、最大日付に0.5日を加えた値は範囲外です。単なる数値文字列を日付へ自動変換はしません。
