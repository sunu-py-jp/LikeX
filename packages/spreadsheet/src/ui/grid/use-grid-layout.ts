...
export function useGridLayout(sheet: SpreadsheetSheet, focusedRow: number, resizing: ColumnWidthPreview | null) {
  const scroller = useRef<HTMLDivElement>(null);
  const [viewport, setViewport] = useState<GridViewport>({ top: 0, height: 480 });
  const columns = columnGeometry(sheet, resizing);
  const rowOffsets = useMemo(() => createRowOffsets({ rowCount: sheet.rowCount, rowHeights: sheet.rowHeights }), [sheet.rowCount, sheet.rowHeights]);
