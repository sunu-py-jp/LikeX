...
function CommentsPanel({ controller: c }: { controller: SpreadsheetController }) {
  ...
  return <aside ref={panel} className="lxs-comments" aria-label="セルのコメント" onCopy={event => event.stopPropagation()} onCut={event => event.stopPropagation()} onPaste={event => event.stopPropagation()}
    onKeyDown={event => { if (event.key === "Escape" && !event.nativeEvent.isComposing && event.keyCode !== 229) { event.preventDefault(); event.stopPropagation(); c.setCommentOpen(false); c.requestGridFocus(); } }}>
    <div className="lxs-object-heading"><strong>{address} のコメント</strong><Command label="コメントを閉じる" onClick={() => { c.setCommentOpen(false); c.requestGridFocus(); }}><Icon name="close" /></Command></div>
    <CommentEditor key={JSON.stringify([c.activeSheet.id, address, comment?.id, comment?.text, comment?.author, c.disabled])} controller={c} address={address} comment={comment} />
  </aside>;
}
...
function CommentEditor({ controller: c, address, comment }: { controller: SpreadsheetController; address: string; comment?: SpreadsheetComment }) {
  ...
  const close = () => { cancelled.current = true; markPending(false); c.setCommentOpen(false); c.requestGridFocus(); };
  if (c.disabled) return <div className="lxs-comment-content">{comment?.author && <p className="lxs-comment-author">{comment.author}</p>}<p>{comment?.text || "このセルにはコメントがありません"}</p></div>;
  return <>
    ...
    <textarea id={inputId} className="lxs-comment-editor" aria-label={`${address} のコメントを編集`} placeholder="コメントを入力" maxLength={SPREADSHEET_LIMITS.commentLength} value={text}
