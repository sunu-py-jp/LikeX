...
export function SpreadsheetGrid({ controller: c }: { controller: SpreadsheetController }) {
  ...
  useEffect(() => {
    ...
    return () => { document.removeEventListener("pointerup", finish); document.removeEventListener("pointercancel", finish); };
  }, []);
    ...
    <div role="grid" aria-label={c.activeSheet.name} aria-readonly={c.disabled} aria-rowcount={c.activeSheet.rowCount + 1} aria-colcount={c.activeSheet.columnCount + 1} aria-multiselectable="true" className="lxs-grid" style={{ width: gridWidth, height: rowOffsets.at(-1) }}>
      <div role="row" aria-rowindex={1} className="lxs-column-headers" style={{ width: gridWidth, height: ROW_HEIGHT }}>
        ...
        {widths.map((width, column) => <div role="columnheader" aria-colindex={column + 2} key={column} className={`lxs-column-header ${column >= bounds.left && column <= bounds.right ? "lxs-header-selected" : ""}`} style={{ width: resizing?.column === column ? resizing.value : width }}>
          <button type="button" className="lxs-header-button" onClick={() => { if (c.commitEdit()) c.selectRange({ row: c.activeSheet.rowCount - 1, column }, { row: 0, column }); }}>{cellAddress(0, column).replace(/\d+$/, "")}</button>
      ...
      {virtualRows.map(row => <div key={row} role="row" aria-rowindex={row + 2} className="lxs-row" style={{ top: rowOffsets[row], height: c.activeSheet.rowHeights?.[row] ?? ROW_HEIGHT, width: gridWidth }}>
        <div role="rowheader" className={`lxs-row-header ${row >= bounds.top && row <= bounds.bottom ? "lxs-header-selected" : ""}`}><button type="button" className="lxs-header-button" onClick={() => { if (c.commitEdit()) c.selectRange({ row, column: c.activeSheet.columnCount - 1 }, { row, column: 0 }); }}>{row + 1}</button></div>
        {widths.map((width, column) => {
          ...
          return <div key={column} role="gridcell" aria-colindex={column + 2} aria-selected={selected} aria-label={`${address}${text ? ` ${text}` : ""}`} title={typeof value === "string" && value.startsWith("#") ? value : undefined}
            ...
            onPointerDown={event => {
              ...
              if (focused) { activeInput.current?.focus({ preventScroll: true }); activeInput.current?.select(); focusIntent.current = false; }
            }}
            onPointerEnter={() => { if (dragging.current) { focusIntent.current = true; c.select({ row, column }, true); } }}
