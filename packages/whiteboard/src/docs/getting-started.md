...
```tsx
import LikeWhiteboard, { createWhiteboard, serializeWhiteboard } from "@likex/whiteboard";
import "@likex/whiteboard/styles.css";

const initial = createWhiteboard({ title: "サンプル" });

<LikeWhiteboard
  initialWhiteboard={initial}
  primaryColor="#496a8f"
  colorMode="system"
  onSave={async model => {
    await saveToYourServer(serializeWhiteboard(model));
  }}
  style={{ height: 640 }}
/>
```
...
| props | 型・挙動 |
| --- | --- |
| `initialWhiteboard` | `WhiteboardModel`。省略すると空のモデル |
...
`WhiteboardHandle`の`getModel()`、`execute(command / commands)`、`select(ids)`、`getSelection()`、`undo()`、`redo()`、`save()`、`discard()`、`importJson(string / Blob)`、`exportJson()`、`exportSvg()`を公開しています。
...
`packages/whiteboard/src`を`components/whiteboard`、`packages/core/src`を`components/core`にコピーします。`core.ts`のimportを`../core`へ、`json.ts`を`../core/json`へ変更してください。React 19と`lucide-react`が必要です。利用側にTailwindの導入は不要です。
