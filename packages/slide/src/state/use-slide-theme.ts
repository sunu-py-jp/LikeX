...
export function useSlideTheme(mode: SlideProps["colorMode"], ownerDocument: Document | null, primaryColor?: string): CSSProperties {
  ...
  return useMemo(() => {
    ...
    return {
      ...Object.fromEntries(Object.entries(isDark ? dark : light).map(([key, value]) => [`--lxp-${key}`, value])),
      ...(palette ? { "--lxp-primary": palette.primary, "--lxp-on-primary": palette.onPrimary,
        "--lxp-primary-hover": palette.primaryHover, "--lxp-accent": palette.accent, "--lxp-selection": palette.selection } : {}),
