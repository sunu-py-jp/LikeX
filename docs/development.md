...
| `npm run pack:library -- --all` | Explorer・Spreadsheet両方のtarballを生成 |
| `npm run test:package -- --all --next` | 両方のtarballを別プロジェクトに導入してNext.jsで検証 |
| `npm run test:copy -- --all --next` | 両方のソースフォルダのコピー導入をNext.jsで検証 |
| `npm run check:release` | 両方のテストから配布・コピー・デモの検証まで実行 |
...
`build:library`、`pack:library`、`test:package`、`test:copy` は既定でExplorerを対象にします。Spreadsheetだけを対象にする場合は `-- --module spreadsheet`、両方なら `-- --all` を付けます。`test:package` の前には同じ対象の `pack:library` を実行してください。Spreadsheetの成果物は `artifacts/spreadsheet/` に生成します。
